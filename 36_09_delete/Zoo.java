...
public class Zoo {
 ...
 "\"ORDER BY ANIMALS.id asc limit 1;";
    ...
    public List<Animal> recuperaAnimals() throws SQLException {
        ...
        String sql = "SELECT animals.nom as ani_nom,animals.id as id_ani,categories.nom as cat_nom FROM ANIMALS, CATEGORIES WHERE animals.categoria=categories.id ORDER BY ani_nom asc;";
    ...
    public void eliminaAnimal(Animal ani)throws SQLException{
        ...
        String sql = "delete from ANIMALS where id="+ani.getId()+";";
        Statement st = null;
    ...
    public void eliminaCategoria(Categoria cat)throws SQLException{
        ...
        try{
            List<Animal> anis = recuperaAnimals();

            for(Animal ani: anis){
                if(ani.getCategoria()==cat){
                    eliminaAnimal(ani);
                }
            }
            st = conn.createStatement();
